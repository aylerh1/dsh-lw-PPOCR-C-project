...
/* Analysis-only REC preprocessing prototype. It writes the same normalized
 * BGR samples directly as NHWC ([1,H,W,C]) instead of materializing NCHW
 * planes. The production recognizer continues to call the NCHW entry point
 * until a complete NHWC graph segment passes end-to-end gates. */
lw_status lw_rec_preprocess_bgr_u8_nhwc(const uint8_t* source, uint64_t source_byte_count,
    ...
    uint32_t output_y;
    const double normalize_scale = 2.0 / 255.0;
    const float padding = (float)(128.0 * normalize_scale - 1.0);
    ...
    for (output_y = 0u; output_y < LW_REC_INPUT_HEIGHT; ++output_y) {
        uint32_t output_x;
        for (output_x = 0u; output_x < target_width; ++output_x) {
            uint32_t channel;
            for (channel = 0u; channel < 3u; ++channel) {
                output[(size_t)(((uint64_t)output_y * target_width + output_x) * 3u + channel)] =
                    padding;
            }
        }
    }
    for (output_y = 0u; output_y < LW_REC_INPUT_HEIGHT; ++output_y) {
        ...
        for (output_x = 0u; output_x < actual_width; ++output_x) {
            ...
            for (channel = 0u; channel < 3u; ++channel) {
                ...
                double value = top + (bottom - top) * weight_y;
                output[(size_t)(((uint64_t)output_y * target_width + output_x) * 3u + channel)] =
                    (float)(value * normalize_scale - 1.0);
            }
        }
    }
    if (resized_width != NULL) {
